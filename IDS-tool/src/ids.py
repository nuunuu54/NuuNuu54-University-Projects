...
def apply_heuristics(row_features: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if up >= PORT_SCAN_UNIQUE_PORTS:
        detections.append(
            {
                "reason": "Port_Scan_Rule",
                "score": min(1.0, up / (PORT_SCAN_UNIQUE_PORTS * 2)),
                "class_guess": "port_scan",
                "explain": {"unique_ports": up},
            }
        )
    ...
    if bf >= BRUTE_FORCE_CONN_THRESHOLD:
        detections.append(
            {
                "reason": "Brute_Force_Rule",
                "score": min(1.0, bf / (BRUTE_FORCE_CONN_THRESHOLD * 2)),
                "class_guess": "brute_force",
                "explain": {"connections_to_same_dst": bf},
            }
        )
    outb = float(row_features.get("outbound_bytes_window", 0))
    inb = float(row_features.get("inbound_bytes_window", 0))
    if (outb >= EXFILTRATION_BYTES_THRESHOLD) or (
        outb / (inb + 1.0) >= EXFILTRATION_RATIO
    ):
        detections.append(
            {
                "reason": "Exfiltration_Rule",
                "score": 1.0,
                "class_guess": "exfiltration",
                "explain": {"outbound_bytes": outb, "inbound_bytes": inb},
            }
        )
    ...
    if (cv <= BEACON_CV_THRESHOLD) and (cnt >= BEACON_MIN_COUNT):
        detections.append(
            {
                "reason": "Beaconing_Rule",
                "score": min(1.0, (BEACON_MIN_COUNT / max(1.0, cnt)) * (1.0 - cv)),
                "class_guess": "beaconing",
                "explain": {"beacon_cv": cv, "recent_count": cnt},
            }
        )
    return detections
...
def run_batch(input_path: Path, model_meta: dict) -> List[Dict[str, Any]]:
    ...
    X, feature_cols = assemble_features(
        df, window_seconds=int(model_meta.get("window_seconds", 60))
    )
    ...
    for i, row in df.reset_index().iterrows():
        ...
        if proba is not None:
            try:
                ...
                class_guess = (
                    label_encoder.classes_[top_idx]
                    ...
                    ml_det.append(
                        {
                            "reason": "ML_Detection",
                            "score": top_prob,
                            "class_guess": str(class_guess),
                            "explain": {"ml_probabilities": probs.tolist()},
                        }
                    )
            except Exception:
                pass
...
def run_streaming(
    input_path: Path, model_meta: dict, delay: float = 0.001
) -> List[Dict[str, Any]]:
    clf = model_meta["model"]
    ...
    for chunk in pd.read_csv(input_path, chunksize=1):
        try:
            ...
            X_buf, feature_cols_buf = assemble_features(
                buf_norm, window_seconds=window_seconds
            )
                    ...
                    ml_det.append(
                        {
                            ...
                            "score": top_prob,
                            "class_guess": str(class_guess),
                            "explain": {"ml_probabilities": probs_last.tolist()},
                        }
                    )
            except Exception:
                ml_det = []
            ...
            new_buf = [
                ...
                if pd.to_datetime(r.get("ts"))
                >= (latest_ts - pd.Timedelta(seconds=window_seconds))
            ]
            buffer_rows = new_buf
        except Exception:
            continue
...
def load_flows_from_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "ts" in df2.columns:
        ...
        df2["ts"] = pd.to_datetime(
            pd.Series(pd.date_range("2000-01-01", periods=len(df2), freq="S"))
        )
    for col in [
        "row_id",
        "src_ip",
        ...
        if col not in df2.columns:
            df2[col] = (
                0
                if col in ("src_port", "dst_port", "bytes", "packets", "duration")
                else ""
            )
...
def main():
    ...
    p.add_argument(
        "--feature-only",
        action="store_true",
        help="Run in feature-only mode (disable windowed host features)",
    )
    args = p.parse_args()
    inp = Path(args.input)
    if not inp.exists():
