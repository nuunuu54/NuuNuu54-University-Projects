...
def main():
    ...
    p.add_argument('--feature-only', action='store_true', help='Train using only per-flow features (disable windowed host features)')
    args = p.parse_args()
    ...
    if label_col is None:
        raise SystemExit('No label column found in input. Add a "label" column with class names.')
    ...
    meta = {'model': clf, 'feature_columns': feature_cols, 'label_encoder': le, 'window_seconds': args.window}
