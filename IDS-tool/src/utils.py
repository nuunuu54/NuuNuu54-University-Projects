...
def load_flows(path: Path) -> pd.DataFrame:
    ...
    if "ts" in df.columns:
        ...
        df["ts"] = pd.to_datetime(
            pd.Series(pd.date_range("2000-01-01", periods=len(df), freq="s"))
        )
        ts_col = "ts"
...
def basic_features(df: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    ...
    flags_dummies = pd.get_dummies(
        df["tcp_flags"].astype(str).fillna(""), prefix="flags"
    )
...
def windowed_host_features(df: pd.DataFrame, window_seconds: int = 60) -> pd.DataFrame:
    ...
    for idx, row in df.reset_index().iterrows():
        ...
        dst = row["dst_ip"]
        dst_port = (
            int(row.get("dst_port", 0)) if not pd.isna(row.get("dst_port", 0)) else 0
        )
        ...
        if len(times) >= 3:
            diffs = (
                np.diff(
                    np.array([t.value for t in pd.to_datetime(times)]).astype("int64")
                )
                / 1e9
            )
            beacon_cv[idx] = (
                float(np.std(diffs) / np.mean(diffs)) if np.mean(diffs) > 0 else 0.0
            )
        else:
            beacon_cv[idx] = 1.0
...
def assemble_features(df: pd.DataFrame, window_seconds: int = 60):
    ...
    feature_cols = [
        c for c in merged.columns if c not in ("row_id", "src_ip", "dst_ip")
    ]
